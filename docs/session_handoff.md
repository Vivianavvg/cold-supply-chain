...
**Done:**
- GCP project created: **project ID `cold-chain-supply`**. Billing linked (required — BigQuery "Sandbox" mode with no billing account blocks service account creation entirely, so this couldn't be avoided for the key-file approach).
- Service account created: `dbt-coldchain` → email `vivi-175@cold-chain-supply.iam.gserviceaccount.com`, roles **BigQuery Data Editor** + **BigQuery Job User**.
- JSON key downloaded and moved (not copied) from `~/Downloads/` to **`C:\Users\Vacav\.dbt\keys\cold-chain-supply-sa.json`** — outside the repo, never committed, confirmed no longer present in Downloads. This file is a live credential; treat it like a password, don't paste its contents anywhere, don't move it back into a synced/repo folder.
  ...
  coldchain_analytics:
    ...
    outputs:
      dev:
        ...
        dataset: dbt_default
        threads: 4
        keyfile: C:/Users/Vacav/.dbt/keys/cold-chain-supply-sa.json
        location: US
  ```
...
3. `raw` dataset created and all 7 CSVs loaded via a new script, **`data_generator/load_to_bigquery.py`** (uses `google.cloud.bigquery` `LoadJobConfig` with `autodetect=True` so columns land as real `TIMESTAMP`/`DATE`/`FLOAT`/`BOOLEAN` types, not all `STRING` — the bronze staging models select raw columns with no casting, so this matters). Run manually, not part of the dbt DAG: `GOOGLE_APPLICATION_CREDENTIALS=<keyfile path> python3 data_generator/load_to_bigquery.py`. Row counts loaded matched the generator's `--seed 42` output exactly (500 shipments, 1004 shipment_legs, 82327 sensor_readings, 1504 shipment_events, plus the 3 small dim-ish tables).
